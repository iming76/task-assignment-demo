...
**Reason**: The product now requests immediate transactional creation once the agent has enough information.

**Migration**: Replace `POST /agent-task/proposals` with `POST /agent-task` and handle its `needs_clarification` or `created` response.

### Requirement: Transactional reviewed apply

**Reason**: The standalone apply phase is replaced by backend-controlled validation and creation within the orchestration request.

**Migration**: Remove calls to `POST /agent-task/apply`; created task trees are returned directly by the successful orchestration response.
